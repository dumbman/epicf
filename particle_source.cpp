...
{
    // todo:
    tube_radius_gt_zero( conf, src_conf );
    tube_axis_start_x_min_rad_ge_zero( conf, src_conf );
    tube_axis_start_x_plus_rad_le_grid_x_size( conf, src_conf );
    tube_axis_start_y_min_rad_ge_zero( conf, src_conf );
    tube_axis_start_y_plus_rad_le_grid_y_size( conf, src_conf );
    tube_axis_start_z_min_rad_ge_zero( conf, src_conf );
    tube_axis_start_z_plus_rad_le_grid_z_size( conf, src_conf );
    tube_axis_end_x_min_rad_ge_zero( conf, src_conf );
    tube_axis_end_x_plus_rad_le_grid_x_size( conf, src_conf );
    tube_axis_end_y_min_rad_ge_zero( conf, src_conf );
    tube_axis_end_y_plus_rad_le_grid_y_size( conf, src_conf );
    tube_axis_end_z_min_rad_ge_zero( conf, src_conf );
    tube_axis_end_z_plus_rad_le_grid_z_size( conf, src_conf );
}
...
void Particle_source_tube::tube_radius_gt_zero( 
    Config &conf, 
    Particle_source_tube_config_part &src_conf )
{
    check_and_exit_if_not( 
...
//
//    check_and_exit_if_not( 
//	src_conf.tube_outer_radius > src_conf.tube_inner_radius,
//	"outer_radius < inner_radius" );

}

void Particle_source_tube::tube_axis_start_x_min_rad_ge_zero( 
    Config &conf, 
    Particle_source_tube_config_part &src_conf )
{
    check_and_exit_if_not( 
...
void Particle_source_tube::tube_axis_start_x_plus_rad_le_grid_x_size( 
    Config &conf, 
    Particle_source_tube_config_part &src_conf )
{
    check_and_exit_if_not( 
	src_conf.tube_axis_start_x + src_conf.tube_outer_radius <= conf.mesh_config_part.grid_x_size,
	"tube_axis_start_x + tube_outer_radius > grid_x_size" );
}

void Particle_source_tube::tube_axis_start_y_min_rad_ge_zero( 
    Config &conf, 
    Particle_source_tube_config_part &src_conf )
{
    check_and_exit_if_not( 
...
void Particle_source_tube::tube_axis_start_y_plus_rad_le_grid_y_size( 
    Config &conf, 
    Particle_source_tube_config_part &src_conf )
{
    check_and_exit_if_not( 
	src_conf.tube_axis_start_y + src_conf.tube_outer_radius <= conf.mesh_config_part.grid_y_size,
	"tube_axis_start_y + tube_outer_radius > grid_y_size" );
}

void Particle_source_tube::tube_axis_start_z_min_rad_ge_zero( 
    Config &conf, 
    Particle_source_tube_config_part &src_conf )
{
    check_and_exit_if_not( 
...
void Particle_source_tube::tube_axis_start_z_plus_rad_le_grid_z_size( 
    Config &conf, 
    Particle_source_tube_config_part &src_conf )
{
    check_and_exit_if_not( 
	src_conf.tube_axis_start_z + src_conf.tube_outer_radius <= conf.mesh_config_part.grid_z_size,
	"tube_axis_start_z + tube_outer_radius > grid_z_size" );
}

void Particle_source_tube::tube_axis_end_x_min_rad_ge_zero( 
    Config &conf, 
    Particle_source_tube_config_part &src_conf )
{
    check_and_exit_if_not( 
...
void Particle_source_tube::tube_axis_end_x_plus_rad_le_grid_x_size( 
    Config &conf, 
    Particle_source_tube_config_part &src_conf )
{
    check_and_exit_if_not( 
	src_conf.tube_axis_end_x + src_conf.tube_outer_radius <= conf.mesh_config_part.grid_x_size,
	"tube_axis_end_x + tube_outer_radius > grid_x_size" );
}

void Particle_source_tube::tube_axis_end_y_min_rad_ge_zero( 
    Config &conf, 
    Particle_source_tube_config_part &src_conf )
{
    check_and_exit_if_not( 
...
void Particle_source_tube::tube_axis_end_y_plus_rad_le_grid_y_size( 
    Config &conf, 
    Particle_source_tube_config_part &src_conf )
{
    check_and_exit_if_not( 
	src_conf.tube_axis_end_y + src_conf.tube_outer_radius <= conf.mesh_config_part.grid_y_size,
	"tube_axis_end_y + tube_outer_radius > grid_y_size" );
}

void Particle_source_tube::tube_axis_end_z_min_rad_ge_zero( 
    Config &conf, 
    Particle_source_tube_config_part &src_conf )
{
    check_and_exit_if_not( 
...
void Particle_source_tube::tube_axis_end_z_plus_rad_le_grid_z_size( 
    Config &conf, 
    Particle_source_tube_config_part &src_conf )
{
    check_and_exit_if_not( 
	src_conf.tube_axis_end_z + src_conf.tube_outer_radius <= conf.mesh_config_part.grid_z_size,
...
void Particle_source_tube::write_hdf5_source_parameters( hid_t current_source_group_id )
{
    ...
    status = H5LTset_attribute_double( current_source_group_id,
				       current_group.c_str(),
    				       "tube_inner_radius", &inner_radius, single_element );
    hdf5_status_check( status );

    status = H5LTset_attribute_double( current_source_group_id,
				       current_group.c_str(),
    				       "tube_outer_radius", &outer_radius, single_element );
    hdf5_status_check( status );
    ...
    std::default_random_engine &rnd_gen )
{
    ...
    Vec3d tube_axis = vec3d_init( ( axis_end_x - axis_start_x ),
				 ( axis_end_y - axis_start_y ),
				 ( axis_end_z - axis_start_z ) );
    double tube_axis_length = vec3d_length( tube_axis );
    double x, y, z;
    double r, phi;
    r = sqrt( random_in_range( inner_radius, outer_radius, rnd_gen ) ) ;
    phi = random_in_range( 0.0, 2.0 * M_PI, rnd_gen );
    z = random_in_range( 0.0, tube_axis_length, rnd_gen );
    //
    x = r * cos( phi );
    y = r * sin( phi );
    z = z;
    Vec3d random_pnt_in_tube_along_z = vec3d_init( x, y, z );
    // rotate:
    // see https://en.wikipedia.org/wiki/Rodrigues'_rotation_formula
    // todo: Too complicated. Try rejection sampling.
    Vec3d random_pnt_in_rotated_tube;
    Vec3d unit_tube_axis = vec3d_normalized( tube_axis );
    Vec3d unit_along_z = vec3d_init( 0, 0, 1.0 );
    Vec3d rotation_axis = vec3d_cross_product( unit_along_z, unit_tube_axis );
    double rotation_axis_length = vec3d_length( rotation_axis );
    if ( rotation_axis_length == 0 ) {
	if ( copysign( 1.0, vec3d_z( unit_tube_axis ) ) >= 0 ){
	    random_pnt_in_rotated_tube = random_pnt_in_tube_along_z;
	} else {
	    random_pnt_in_rotated_tube = vec3d_negate( random_pnt_in_tube_along_z );
	}
    } else {
	Vec3d unit_rotation_axis = vec3d_normalized( rotation_axis );
	double rot_cos = vec3d_dot_product( unit_tube_axis, unit_along_z );
	double rot_sin = rotation_axis_length;
	
	random_pnt_in_rotated_tube =
	    vec3d_add(
		vec3d_times_scalar( random_pnt_in_tube_along_z, rot_cos ),
		vec3d_add(
		    vec3d_times_scalar(
			vec3d_cross_product( unit_rotation_axis,
					     random_pnt_in_tube_along_z ),
			rot_sin ),
		    vec3d_times_scalar(
			unit_rotation_axis,
			( 1 - rot_cos ) * vec3d_dot_product(
    ...
    // shift:
    Vec3d shifted = vec3d_add( random_pnt_in_rotated_tube,
			       vec3d_init( axis_start_x, axis_start_y, axis_start_z ) );
    return shifted;
}
